function [data,stats] = PhasePrecession_IZ(positions,spikes,phases,varargin)
...
if ~isempty(positions) && nargout >= 2,
    ...
    lindat = (x(ok)-fieldStart)./(fieldStop-fieldStart);
	[beta,r2,p] = CircularRegression(lindat,data.position.phase(ok),slope);    
