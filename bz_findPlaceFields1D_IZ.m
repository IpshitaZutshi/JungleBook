function [placeFieldStats] = bz_findPlaceFields1D_IZ(varargin)
...
for unit = 1:length(firingMaps.rateMaps)     
    for c = 1:4%length(firingMaps.rateMaps{1})
        % Default values
        ...
        while true,
            ...
            if i>1
                ...
                if isempty(field0ini), field0ini = 1; end % Added by IZ
                ...
                if idxBetwFields == 1 %If 1, that means field1 is AFTER field 0
...
if saveMat
   if ~downsample
       ...
       save([basepath,filesep,placeFieldStats.sessionName '_DS.placeFields.cellinfo.mat'],'placeFieldStats'); 
   end
end
...
for unit = 1:length(firingMaps.rateMaps)
    ...
    for c = 1:length(firingMaps.rateMaps{1})
        ...
        if sum(firingMaps.rateMaps{unit}{c})>0 && c < 5
            hold on
    ...
    if ~downsample
        if ~isfolder([basepath,filesep,'newPCs'])
            mkdir(basepath,'newPCs')
        end
        saveas(gcf,[basepath,filesep,'newPCs',filesep ,'cell_' num2str(unit) '.png'],'png');  
        saveas(gcf,[basepath,filesep,'newPCs',filesep ,'cell_' num2str(unit) '.fig'],'fig'); 
    else
        if ~isfolder([basepath,filesep,'DSPCs'])
            mkdir(basepath,'DSPCs')
        end
        saveas(gcf,[basepath,filesep,'DSPCs',filesep ,'cell_' num2str(unit) '.png'],'png'); 
        saveas(gcf,[basepath,filesep,'DSPCs',filesep ,'cell_' num2str(unit) '.fig'],'fig'); 
    end
    close all;
end
