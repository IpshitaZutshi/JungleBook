function ripples = SessBehaviorRipplesLowThresh(varargin)
...
if exist('Summ\mazeRipplesLowThresh.mat','file') && ~force 
    disp('Ripples already computed! Loading file.');
    load('Summ\mazeRipplesLowThresh.mat');
else
    for rr = 1:3
        for cc = 1:2
            for zz = 1:6
        ...
        for jj = 1:length(efields)
            ...
            for zz = 1:6
                ...
                if (zz == 3 || zz == 6) && sessionArmChoice.(efields{jj}).delay.dur < 1 
                    ...
                    keepIdx = InIntervals(ripplesLowThresh.peaks,events');    
                end
                mazeRipples.number{region,target}{zz} = catpad(3,mazeRipples.number{region,target}{zz},sum(keepIdx));    
                mazeRipples.duration{region,target}{zz} = catpad(3,mazeRipples.duration{region,target}{zz},ripplesLowThresh.data.duration(keepIdx,:)); 
                mazeRipples.peakPower{region,target}{zz} = catpad(3,mazeRipples.peakPower{region,target}{zz},ripplesLowThresh.data.peakAmplitude(keepIdx,:)); 
                mazeRipples.frequency{region,target}{zz} = catpad(3,mazeRipples.frequency{region,target}{zz},ripplesLowThresh.data.peakFrequency(keepIdx,:)); 
                mazeRipples.timestamps{region,target}{zz} = catpad(3,mazeRipples.timestamps{region,target}{zz},ripplesLowThresh.timestamps(keepIdx,:)); 
            end        
    ...
    if saveMat
        save([expPath '\Summ\' 'mazeRipplesLowThresh.mat'], 'mazeRipples');
    end
end
