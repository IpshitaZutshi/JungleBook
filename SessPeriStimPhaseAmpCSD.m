function ModData = SessPeriStimPhaseAmpCSD(varargin)
...
if exist(strcat('Summ\PhaseAmpCSD.mat'),'file') && ~force 
    ...
    for ii = 1:size(allSess,1)
        ...
        mlCh = hippocampalLayers.ml; 
        channelOrder = hippocampalLayers.channelOrder;
        chanList = [pyrCh radCh slmCh mlCh];
        ...
        for jj = 1:length(efields)
            ...
            if exist([efields{jj} '\PhaseAmpCSD.mat'],'file') && ~force 
                disp(strcat('Phase amp for sess ',num2str(ii),' already computed! Loading file.'));
                load([efields{jj} '\PhaseAmpCSD.mat']);
            else
                for zz = [1 2 4 5]
                    ...
                    for ch = 1:length(chName)
                        ...
                        if ~isnan(chanList(ch))
                            ...
                            if (zz == 3 || zz == 6) && sessionArmChoice.(efields{jj}).delay.dur < 1 
                                comod.(chName{ch}){3}.map = nan;
                            else
                                comod.(chName{ch}){zz} = bz_PhaseAmplitudeDist_IZ(lfpCh,lfpPyrCh,phaserange,amprange,'intervals',events','makePlot',false);  
                            end
                        else
                            comod.(chName{ch}){zz}.map = nan;
                        end
                    end
                end                
            end
            for ch = 1:length(chName)
                for zz = [1 2 4 5]            
                   ModData.(chName{ch}){region,target}{zz} =  cat(3,ModData.(chName{ch}){region,target}{zz},comod.(chName{ch}){zz}.map);
                   if ch == 1
                       phasecenters = comod.(chName{ch}){zz}.phasecenters;
                       amp_freq =  comod.(chName{ch}){zz}.amp_freq;
                   end
                end
            end

            if saveMat
                save([efields{jj} '\PhaseAmpCSD.mat'], 'comod');
            end
    ...
    if saveMat
        save([expPath '\Summ\' 'PhaseAmpCSD.mat'], 'ModData','phasecenters','amp_freq');
    end
end
