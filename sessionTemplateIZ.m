...
addParameter(p,'importSkippedChannels',false,@islogical); % Import skipped channels from the xml as bad channels
addParameter(p,'importSyncedChannels',false,@islogical); % Import channel not synchronized between anatomical and spike groups as bad channels
...
if ~isfield(session,'animal')
    session.animal.name = pathPieces{end-2}; % Animal name is inferred from the data path
