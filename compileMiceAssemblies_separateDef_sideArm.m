function compileMiceAssemblies_separateDef_sideArm(varargin)
...
for tag = 1:3
    ...
    for aa = 1:length(animalID)    
        ...
        if exist(strcat('Summ\AssembliesSeparateSideArm.mat'),'file')
        ...
        if tag == 1
            if aa < 13 % non CA3 mice
                if ~isempty(Assemblies.expressionBase{2,2})
                    baseManip{1} = [baseManip{1}; -(Assemblies.expressionBase{2,target}(:,3)- Assemblies.expressionBase{2,target}(:,4))./(Assemblies.expressionBase{2,target}(:,3)+ Assemblies.expressionBase{2,target}(:,4))];
                    mecManip{1} = [mecManip{1}; -(Assemblies.expressionBase{2,target}(:,1)-Assemblies.expressionBase{2,target}(:,2))./(Assemblies.expressionBase{2,target}(:,1)+Assemblies.expressionBase{2,target}(:,2))];                    
                end
                if ~isempty(Assemblies.expressionStim{2,2})
                    baseManip{2} = [baseManip{2}; -(Assemblies.expressionStim{2,target}(:,2)- Assemblies.expressionStim{2,target}(:,1))./(Assemblies.expressionStim{2,target}(:,2)+ Assemblies.expressionStim{2,target}(:,1))];
                    mecManip{2} = [mecManip{2}; -(Assemblies.expressionStim{2,target}(:,3)-Assemblies.expressionStim{2,target}(:,4))./(Assemblies.expressionStim{2,target}(:,3)+Assemblies.expressionStim{2,target}(:,4))];
                end  
                nummecbase = [nummecbase Assemblies.numberBase{2,target}];
                nummec = [nummec Assemblies.numberStim{2,target}];

                nummecbaseA = [nummecbaseA Assemblies.numberBaseA{2,target}];
                nummecA = [nummecA Assemblies.numberStimA{2,target}];                
            else
        ...
        if tag == 2
            if ~isempty(Assemblies.expressionBase{1,target})
                contraBaseManip{1} = [contraBaseManip{1}; -(Assemblies.expressionBase{1,target}(:,3)-Assemblies.expressionBase{1,target}(:,4))./(Assemblies.expressionBase{1,target}(:,3)+Assemblies.expressionBase{1,target}(:,4))];
                contraBaseManip{2} = [contraBaseManip{2}; -(Assemblies.expressionStim{1,target}(:,2)-Assemblies.expressionStim{1,target}(:,1))./(Assemblies.expressionStim{1,target}(:,2)+Assemblies.expressionStim{1,target}(:,1))];
                contraManip{1} = [contraManip{1}; -(Assemblies.expressionBase{1,target}(:,1)-Assemblies.expressionBase{1,target}(:,2))./(Assemblies.expressionBase{1,target}(:,1)+Assemblies.expressionBase{1,target}(:,2))];
                contraManip{2} = [contraManip{2}; -(Assemblies.expressionStim{1,target}(:,3)-Assemblies.expressionStim{1,target}(:,4))./(Assemblies.expressionStim{1,target}(:,3)+Assemblies.expressionStim{1,target}(:,4))];                
                
                numcontrabase= [numcontrabase Assemblies.numberBase{1,target}];
                numcontra= [numcontra Assemblies.numberStim{1,target}];
                
                numcontrabaseA= [numcontrabaseA Assemblies.numberBaseA{1,target}];
                numcontraA= [numcontraA Assemblies.numberStimA{1,target}];                
            end
            if ~isempty(Assemblies.expressionBase{3,2})
                biBaseManip{1} = [biBaseManip{1}; -(Assemblies.expressionBase{3,target}(:,3)-Assemblies.expressionBase{3,target}(:,4))./(Assemblies.expressionBase{3,target}(:,3)+Assemblies.expressionBase{3,target}(:,4))];
                biBaseManip{2} = [biBaseManip{2}; -(Assemblies.expressionStim{3,target}(:,2)-Assemblies.expressionStim{3,target}(:,1))./(Assemblies.expressionStim{3,target}(:,2)+Assemblies.expressionStim{3,target}(:,1))];
                biManip{1} = [biManip{1}; -(Assemblies.expressionBase{3,target}(:,1)-Assemblies.expressionBase{3,target}(:,2))./(Assemblies.expressionBase{3,target}(:,1)+Assemblies.expressionBase{3,target}(:,2))];
                biManip{2} = [biManip{2}; -(Assemblies.expressionStim{3,target}(:,3)-Assemblies.expressionStim{3,target}(:,4))./(Assemblies.expressionStim{3,target}(:,3)+Assemblies.expressionStim{3,target}(:,4))];                
                
                numbibase = [numbibase Assemblies.numberBase{3,target}];
                numbi = [numbi Assemblies.numberStim{3,target}];

                numbibaseA = [numbibaseA Assemblies.numberBaseA{3,target}];
                numbiA = [numbiA Assemblies.numberStimA{3,target}];                
            end         
        end
        
        if tag == 3
            if ~isempty(Assemblies.expressionBase{1,target})
                ca1baseManip{1} = [ca1baseManip{1}; -(Assemblies.expressionBase{1,target}(:,3)-Assemblies.expressionBase{1,target}(:,4))./(Assemblies.expressionBase{1,target}(:,3)+Assemblies.expressionBase{1,target}(:,4))];
                ca1baseManip{2} = [ca1baseManip{2}; -(Assemblies.expressionStim{1,target}(:,2)-Assemblies.expressionStim{1,target}(:,1))./(Assemblies.expressionStim{1,target}(:,2)+Assemblies.expressionStim{1,target}(:,1))];
                ca1Manip{1} = [ca1Manip{1}; -(Assemblies.expressionBase{1,target}(:,1)-Assemblies.expressionBase{1,target}(:,2))./(Assemblies.expressionBase{1,target}(:,1)+Assemblies.expressionBase{1,target}(:,2))];
                ca1Manip{2} = [ca1Manip{2}; -(Assemblies.expressionStim{1,target}(:,3)-Assemblies.expressionStim{1,target}(:,4))./(Assemblies.expressionStim{1,target}(:,3)+Assemblies.expressionStim{1,target}(:,4))];                
                
                numca1base = [numca1base Assemblies.numberBase{1,target}];
                numca1 = [numca1 Assemblies.numberStim{1,target}];   

                numca1baseA = [numca1baseA Assemblies.numberBaseA{1,target}];
                numca1A = [numca1A Assemblies.numberStimA{1,target}];                   
            end
            if ~isempty(Assemblies.expressionBase{3,target})
                mecca1baseManip{1} = [mecca1baseManip{1}; -(Assemblies.expressionBase{3,target}(:,3)-Assemblies.expressionBase{3,target}(:,4))./(Assemblies.expressionBase{3,target}(:,3)+Assemblies.expressionBase{3,target}(:,4))];
                mecca1baseManip{2} = [mecca1baseManip{2}; -(Assemblies.expressionStim{3,target}(:,2)-Assemblies.expressionStim{3,target}(:,1))./(Assemblies.expressionStim{3,target}(:,2)+Assemblies.expressionStim{3,target}(:,1))];
                mecca1Manip{1} = [mecca1Manip{1}; -(Assemblies.expressionBase{3,target}(:,1)-Assemblies.expressionBase{3,target}(:,2))./(Assemblies.expressionBase{3,target}(:,1)+Assemblies.expressionBase{3,target}(:,2))];
                mecca1Manip{2} = [mecca1Manip{2}; -(Assemblies.expressionStim{3,target}(:,3)-Assemblies.expressionStim{3,target}(:,4))./(Assemblies.expressionStim{3,target}(:,3)+Assemblies.expressionStim{3,target}(:,4))];                                       
                
                nummecca1base = [nummecca1base Assemblies.numberBase{3,target}];
                nummecca1 = [nummecca1 Assemblies.numberStim{3,target}];   
                
                nummecca1baseA = [nummecca1baseA Assemblies.numberBaseA{3,target}];
                nummecca1A = [nummecca1A Assemblies.numberStimA{3,target}];                  
            end               
        end        
...
subplot(3,5,6:10)
[stats.ExpressionmEC] = groupStats([baseManip(1) baseManip(2) mecManip(2) mecManip(1)],[],'doPlot',true,'inAxis',true,'plotType','boxplot');
