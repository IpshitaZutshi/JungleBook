...
for ii=1:size(pathToSessionsAll,2)
    ...
    [stccg, t] = CCG({ripples.peaks st},[],'binSize',0.01,'duration',1);
    ...
    for i = analogCh
        ...
        if sum(keepIdx(:,1))>10
            [stccg, t] = CCG({ripples.peaks st(keepIdx(:,1)) },[],'binSize',0.01,'duration',1);            
        ...
        if sum(keepIdx(:,2))>10
            [stccg, t] = CCG({ripples.peaks st(keepIdx(:,2)) },[],'binSize',0.01,'duration',1);            
        ...
        if sum(keepIdx(:,3))>10
            [stccg, t] = CCG({ripples.peaks st(keepIdx(:,3)) },[],'binSize',0.01,'duration',1);            
...
subplot(4,6,1)
imagesc(t,1:1:size(distRipAll,1),zscore(distRipAll,[],2))
colormap(YlGnBu)
colorbar    
caxis([-2 2])
subplot(4,6,7)
...
%     stats.ripPre{ii} = groupStats([{a(:,1)} {a(:,2)} {a(:,3)}],[1 2 3],'inAxis',true,'plotType','BoxLinesSEM','repeatedMeasures',true);
