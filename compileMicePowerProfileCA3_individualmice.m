function PD = compileMicePowerProfileCA3_individualmice
...
if strcmp(tag(4:end),'Saline') == 1
    ...
    mice = {'IZ33\Final','IZ32\Final'};%,'IZ29\Final'};
end
reg = {'CA3','mEC','Both'};
...
%                     chanImp = chanImp(2:end);
%                 else      
%                     chanImp = chanImp(Chstart-1:end);
%                 end
...
%                 chanImp = chanImp(chanImp<=62);
               ...
               % x = 1:(68/length(chanImp)):68; %Determine current sampling                     
                ...
                subplot(6,4*length(mice),(4*length(mice)*(rem(kk,3))+4*(m-1)+1))
                hold on
...
%                 stdProfile = interp1(x,stdProfile,nC);  
                ...
                subplot(6,4*length(mice),(4*length(mice)*(rem(kk,3))+4*(m-1)+2))
                hold on
                % Interpolate so that each profile has 64 datapoints
...
%                 stdProfile = interp1(x,stdProfile,nC);
                ...
                subplot(6,4*length(mice),(4*length(mice)*(rem(kk,3))+4*(m-1)+3))
                hold on
                % Interpolate so that each profile has 64 datapoints
...
%                 stdProfile = interp1(x,stdProfile,nC);                
                ...
                subplot(6,4*length(mice),(4*length(mice)*(rem(kk,3))+4*(m-1)+4))
                hold on
                % Interpolate so that each profile has 64 datapoints
...
%                     nC = 1:1:length(chanImp);  
                    ...
                    subplot(6,4*length(mice),(4*length(mice)*(rem(kk,3))+12*length(mice)+4*(m-1)+1))
                    hold on
                    Meanprofile = meanTheta(chanImp)';
                    stdProfile = stdTheta(chanImp)'; 
%                     Meanprofile = interp1(x,Meanprofile,nC);
%                     stdProfile = interp1(x,stdProfile,nC);                      
                    ...
                    subplot(6,4*length(mice),(4*length(mice)*(rem(kk,3))+12*length(mice)+4*(m-1)+2))
                    hold on
                    Meanprofile = meansg(chanImp)';
                    stdProfile = stdsg(chanImp)'; 
%                     Meanprofile = interp1(x,Meanprofile,nC);
%                     stdProfile = interp1(x,stdProfile,nC);                      
                    ...
                    subplot(6,4*length(mice),(4*length(mice)*(rem(kk,3))+12*length(mice)+4*(m-1)+3))
                    hold on
                    Meanprofile = meanmg(chanImp)';
                    stdProfile = stdmg(chanImp)';                     
%                     Meanprofile = interp1(x,Meanprofile,nC);
%                     stdProfile = interp1(x,stdProfile,nC);                    
                    ...
                    subplot(6,4*length(mice),(4*length(mice)*(rem(kk,3))+12*length(mice)+4*(m-1)+4))
                    hold on
                    Meanprofile = meanhfo(chanImp)';
                    stdProfile = stdhfo(chanImp)';                     
%                     Meanprofile = interp1(x,Meanprofile,nC);
