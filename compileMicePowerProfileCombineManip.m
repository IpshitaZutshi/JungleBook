function PD = compileMicePowerProfileCombineManip

mice = {'IZ21\Final'};%,'IZ25\Final','IZ26\Final','IZ26\Final'};%If in a 'Final' Folder, include the Final in the string
shankNum = [1,1,1,3];
    ...
    for jj = 1%:length(target)
        ...
        for ii = 1:length(reg)
            ...
            for kk = 1:length(zone)      
                ...
                if kk<=3
                    baseTheta{kk} = catpad(3,baseTheta{kk},PowerProfile.theta{ii,jj}{kk}(:,:,2:end));
                    basesg{kk} = catpad(3,basesg{kk},PowerProfile.sg{ii,jj}{kk}(:,:,2:end));
                    basemg{kk} = catpad(3,basemg{kk},PowerProfile.mg{ii,jj}{kk}(:,:,2:end));
                    basehfo{kk} = catpad(3,basehfo{kk},PowerProfile.hfo{ii,jj}{kk}(:,:,2:end));
                    continue
                end
