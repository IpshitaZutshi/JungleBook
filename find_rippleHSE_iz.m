function ripple_HSE = find_rippleHSE_iz(basepath, ripples, varargin)
...
addParameter(p,'useVel',false); % the channels used for decoding, eg [ 1 2 3 4 5]
addParameter(p,'intervals',[]); %

parse(p,varargin{:});
interval = p.Results.interval;
save_evt = p.Results.save_evt;
save_fig = p.Results.save_fig;
useVel = p.Results.useVel;
...
if ~useVel
    ...
    v = abs([0 diff(behavior.position.lin(tidcs))'])*30; 
    v(v>60) = nan;
    %v = conv(v,fspecial('gauss',[1 round(3*30)],30),'same'); % Filter at 1Hz standard
    v = sqrt(nansum(v.^2,1)); % Euclidean distance

    hse_ts = nanmean(HSE.timestamps,2);
    for ii = 1:length(hse_ts)
        [~,idx] = min(abs(behavior.timestamps(tidcs)-hse_ts(ii)));
        vel(ii) = v(idx);
    end
    rippleHSE_idx = find(vel<5);
end
ripple_HSE = struct();
