function [comod] = bz_ModIndex_IZ(lfp,varargin)
...
%Parameters
p = inputParser;
addParameter(p,'flagPlot',true,@islogical);
addParameter(p,'intervals',[-Inf Inf]);
addParameter(p,'phaserange',[2:0.2:20]);
addParameter(p,'amprange',[30:1:300]);
addParameter(p,'useCSD',false,@islogical);
addParameter(p,'lfpPyrCh',[]);
parse(p,varargin{:});

flagPlot = p.Results.flagPlot;
intervals = p.Results.intervals;
phaserange = p.Results.phaserange;
amprange = p.Results.amprange;
useCSD = p.Results.useCSD;
lfpPyrCh = p.Results.lfpPyrCh;
...
for bnd = 1:length(phaserange)-1
    if ~useCSD
        filtered_phase(bnd,:) = bz_Filter(lfp,'passband',phaserange(bnd:bnd+1),'filter','fir1','intervals',intervals);
    else
        filtered_phase(bnd,:) = bz_Filter(lfpPyrCh,'passband',phaserange(bnd:bnd+1),'filter','fir1','intervals',intervals);
    end
end
...
for apr = 1:length(amprange)-1
    wavespec_amp = bz_WaveSpec(lfp,'frange',[amprange(apr) amprange(apr+1)],'nfreqs',1,'intervals',intervals);
...
%     axis xy
    ...
    figure
    imagesc(phaserange(2:end),amprange(2:end),comod)
    set(gca,'YDir','normal')
    colormap jet
    xlabel('Frequency phase');
    ylabel('Frequency amplitude');
