function PD = compileMicePowerProfileCA1

mice = {'IZ15\Final','IZ18\Final','IZ20\Final','IZ21\Final','IZ23\Final'};%If in a 'Final' Folder, include the Final in the string
shankNum = [3,1,1,1,3];
tag = 'CA1';
    ...
    for jj = 1%:length(target)
        for ii = 1:length(reg)
            ...
            for kk = 1:length(zone)      
                ...
                if strcmp(mice{m},'IZ15\Final') == 1 
                    chanImp = chanImp(3:(end-5));
                    nC = 1:1:32;     
                    x = 1:(38/length(chanImp)):38; 
                elseif strcmp(mice{m},'IZ18\Final') == 1
                    chanImp = chanImp(Chstart-5:end);
                    nC = 1:1:32;     
                    x = 1:(38/length(chanImp)):38;                     
                elseif strcmp(mice{m},'IZ23\Final') == 1 
                    chanImp = chanImp(3:end);
                    nC = 1:1:64;     
                    x = 1:(68/length(chanImp)):68; 
                elseif strcmp(mice{m},'IZ20\Final') == 1 
                    chanImp = chanImp(Chstart-5:end);
                    nC = 1:1:64;     
                    x = 1:(68/length(chanImp)):68;                           
                elseif strcmp(mice{m},'IZ21\Final') == 1    
                    chanImp = chanImp(Chstart-5:end-10);
                    nC = 1:1:64;     
                    x = 1:(68/length(chanImp)):68;                       
                end                
               ...
               if kk == 1 || kk ==2
                    ...
                    PD.Theta{ii,kk} = catpad(2,PD.Theta{ii,kk},Meanprofile);
                    ...
                    Meanprofile = smooth(Meanprofile); 
                    stdProfile = smooth(stdProfile);
                    PD.sg{ii,kk} = catpad(2,PD.sg{ii,kk},Meanprofile);    
                    dev1 = Meanprofile-stdProfile;
                    dev2 = Meanprofile+stdProfile;
                    fill([dev1' flip(dev2')],[nC flip(nC)],col,'FaceAlpha',.2,'EdgeColor','none')
                    plot(Meanprofile,nC,'color',col,'LineWidth',1.5); 
                    ...
                    Meanprofile = smooth(Meanprofile); 
                    stdProfile = smooth(stdProfile);
                    PD.mg{ii,kk} = catpad(2,PD.mg{ii,kk},Meanprofile); 
                    dev1 = Meanprofile-stdProfile;
                    dev2 = Meanprofile+stdProfile;
                    fill([dev1' flip(dev2')],[nC flip(nC)],col,'FaceAlpha',.2,'EdgeColor','none')
                    plot(Meanprofile,nC,'color',col,'LineWidth',1.5); 
                    ...
                    Meanprofile = smooth(Meanprofile);                    
                    stdProfile = smooth(stdProfile);
                    PD.hfo{ii,kk} = catpad(2,PD.hfo{ii,kk},Meanprofile);  
                    dev1 = Meanprofile-stdProfile;
                    dev2 = Meanprofile+stdProfile;
                    fill([dev1' flip(dev2')],[nC flip(nC)],col,'FaceAlpha',.2,'EdgeColor','none')
                    plot(Meanprofile,nC,'color',col,'LineWidth',1.5); 
...
figure(jj)
col = [115/243 82/243 66/243; 56/243 61/243 150/243; 52/243 52/243 52/243];
nC = 1:1:64;
