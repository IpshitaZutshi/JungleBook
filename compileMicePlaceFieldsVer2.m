...
function [PF, sessCorr] = compileMicePlaceFieldsVer2(varargin)
...
%             sessCorr{ii,jj}(:,4) = diag(corr(PF.rateMap{ii,jj}{1,3}(idxSelect,stem:95)',PF.rateMap{ii,jj}{1,4}(idxSelect,stem:95)','Rows','pairwise','Type','Spearman'));            
            if ~useZScore
                sessCorr{ii,jj}(:,1) = diag(corr(PF.sessCorrMap{ii,jj}{2}(idxSelect,10:ret)',PF.sessCorrMap{ii,jj}{4}(idxSelect,10:ret)','Rows','pairwise','Type','Spearman'));
                sessCorr{ii,jj}(:,2) = diag(corr(PF.sessCorrMap{ii,jj}{2}(idxSelect,stem:95)',PF.sessCorrMap{ii,jj}{4}(idxSelect,stem:95)','Rows','pairwise','Type','Spearman'));
                sessCorr{ii,jj}(:,3) = diag(corr(PF.sessCorrMap{ii,jj}{6}(idxSelect,10:ret)',PF.sessCorrMap{ii,jj}{8}(idxSelect,10:ret)','Rows','pairwise','Type','Spearman'));
                sessCorr{ii,jj}(:,4) = diag(corr(PF.sessCorrMap{ii,jj}{6}(idxSelect,stem:95)',PF.sessCorrMap{ii,jj}{8}(idxSelect,stem:95)','Rows','pairwise','Type','Spearman'));
                ...
                sessCorr{ii,jj}(:,1) = diag(corr(zscore(PF.sessCorrMap{ii,jj}{2}(idxSelect,10:ret),[],2)',zscore(PF.sessCorrMap{ii,jj}{4}(idxSelect,10:ret),[],2)','Rows','pairwise','Type','Spearman'));
                sessCorr{ii,jj}(:,2) = diag(corr(zscore(PF.sessCorrMap{ii,jj}{2}(idxSelect,stem:95),[],2)',zscore(PF.sessCorrMap{ii,jj}{4}(idxSelect,stem:95),[],2)','Rows','pairwise','Type','Spearman'));
                sessCorr{ii,jj}(:,3) = diag(corr(zscore(PF.sessCorrMap{ii,jj}{6}(idxSelect,10:ret),[],2)',zscore(PF.sessCorrMap{ii,jj}{8}(idxSelect,10:ret),[],2)','Rows','pairwise','Type','Spearman'));
                sessCorr{ii,jj}(:,4) = diag(corr(zscore(PF.sessCorrMap{ii,jj}{6}(idxSelect,stem:95),[],2)',zscore(PF.sessCorrMap{ii,jj}{8}(idxSelect,stem:95),[],2)','Rows','pairwise','Type','Spearman'));
            ...
            if ~useZScore
                for m = unique(PF.mice{ii,jj}{1})'
                    ...
                    corrstim(2,:,m) = diag(corr(PF.sessCorrMap{ii,jj}{6}(idxMouse,:),PF.sessCorrMap{ii,jj}{8}(idxMouse,:),'Rows','pairwise','Type','Spearman'));
                end
            else 
                clear temp
                for nn = 1:8
    ...
    for jj = 1%:length(target)  
        ...
        for ii = 2:3
            ...
            for zz = 1:4               
                ...
                if zz == 1 || zz ==3
                    PFsorting = PF.rateMap{ii,jj}{1,zz}(idxSelect,:);
                    [~,idx] = max(PFsorting,[],2);
                    [~,sortidx] = sort(idx,'ascend');
                    PFbase = [PFbase;PFtemp(sortidx,:)];
                end
        ...
        if ~useZScore
            ...
            sessCorrRet{jj}(:,3) = diag(corr(PF.sessCorrMap{2,jj}{2}(idxSelect,10:ret)',PF.sessCorrMap{2,jj}{4}(idxSelect,10:ret)','Rows','pairwise','Type','Spearman'));
            sessCorrRet{jj}(:,4) = diag(corr(PF.sessCorrMap{2,jj}{6}(idxSelect,10:ret)',PF.sessCorrMap{2,jj}{8}(idxSelect,10:ret)','Rows','pairwise','Type','Spearman'));
            ...
            sessCorr{jj}(:,1) = diag(corr(PF.sessCorrMap{2,jj}{1}(idxSelect,stem:100)',PF.sessCorrMap{2,jj}{2}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,2) = diag(corr(PF.sessCorrMap{2,jj}{5}(idxSelect,stem:100)',PF.sessCorrMap{2,jj}{6}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,3) = diag(corr(PF.sessCorrMap{2,jj}{2}(idxSelect,stem:100)',PF.sessCorrMap{2,jj}{4}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,4) = diag(corr(PF.sessCorrMap{2,jj}{6}(idxSelect,stem:100)',PF.sessCorrMap{2,jj}{8}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,5) = diag(corr(PF.sessCorrMap{2,jj}{2}(idxSelect,stem:100)',PF.sessCorrMap{3,jj}{2}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,6) = diag(corr(PF.sessCorrMap{2,jj}{6}(idxSelect,stem:100)',PF.sessCorrMap{3,jj}{6}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,7) = diag(corr(PF.sessCorrMap{2,jj}{2}(idxSelect,stem:100)',PF.sessCorrMap{3,jj}{4}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,8) = diag(corr(PF.sessCorrMap{2,jj}{6}(idxSelect,stem:100)',PF.sessCorrMap{3,jj}{8}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,9) = diag(corr(PF.sessCorrMap{3,jj}{1}(idxSelect,stem:100)',PF.sessCorrMap{3,jj}{3}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,10) = diag(corr(PF.sessCorrMap{3,jj}{5}(idxSelect,stem:100)',PF.sessCorrMap{3,jj}{7}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,11) = diag(corr(PF.sessCorrMap{3,jj}{1}(idxSelect,stem:100)',PF.sessCorrMap{3,jj}{2}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,12) = diag(corr(PF.sessCorrMap{3,jj}{5}(idxSelect,stem:100)',PF.sessCorrMap{3,jj}{6}(idxSelect,stem:100)','Rows','pairwise','Type','Spearman'));
        else
            ...
            sessCorrRet{jj}(:,3) = diag(corr(zscore(PF.sessCorrMap{2,jj}{2}(idxSelect,10:ret),[],2)',zscore(PF.sessCorrMap{2,jj}{4}(idxSelect,10:ret),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorrRet{jj}(:,4) = diag(corr(zscore(PF.sessCorrMap{2,jj}{6}(idxSelect,10:ret),[],2)',zscore(PF.sessCorrMap{2,jj}{8}(idxSelect,10:ret),[],2)','Rows','pairwise','Type','Spearman'));
            ...
            sessCorrRet{jj}(:,9) = diag(corr(zscore(PF.sessCorrMap{3,jj}{2}(idxSelect,10:ret),[],2)',zscore(PF.sessCorrMap{3,jj}{4}(idxSelect,10:ret),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorrRet{jj}(:,10) = diag(corr(zscore(PF.sessCorrMap{3,jj}{6}(idxSelect,10:ret),[],2)',zscore(PF.sessCorrMap{3,jj}{8}(idxSelect,10:ret),[],2)','Rows','pairwise','Type','Spearman'));

            sessCorr{jj}(:,1) = diag(corr(zscore(PF.sessCorrMap{2,jj}{1}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{2,jj}{2}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,2) = diag(corr(zscore(PF.sessCorrMap{2,jj}{5}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{2,jj}{6}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,3) = diag(corr(zscore(PF.sessCorrMap{2,jj}{2}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{2,jj}{4}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,4) = diag(corr(zscore(PF.sessCorrMap{2,jj}{6}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{2,jj}{8}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,5) = diag(corr(zscore(PF.sessCorrMap{2,jj}{2}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{3,jj}{2}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,6) = diag(corr(zscore(PF.sessCorrMap{2,jj}{6}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{3,jj}{6}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,7) = diag(corr(zscore(PF.sessCorrMap{2,jj}{2}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{3,jj}{4}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,8) = diag(corr(zscore(PF.sessCorrMap{2,jj}{6}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{3,jj}{8}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,9) = diag(corr(zscore(PF.sessCorrMap{3,jj}{2}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{3,jj}{4}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
            sessCorr{jj}(:,10) = diag(corr(zscore(PF.sessCorrMap{3,jj}{6}(idxSelect,stem:85),[],2)',zscore(PF.sessCorrMap{3,jj}{8}(idxSelect,stem:85),[],2)','Rows','pairwise','Type','Spearman'));
        ...
        if ~useZScore
            for m = unique(PF.mice{ii,jj}{1})'
                ...
                corrBothCA3(1,:,m) = diag(corr(PF.sessCorrMap{3,jj}{2}(idxMouse,:),PF.sessCorrMap{3,jj}{4}(idxMouse,:),'Rows','pairwise','Type','Spearman'));
                corrBothCA3(2,:,m) = diag(corr(PF.sessCorrMap{3,jj}{6}(idxMouse,:),PF.sessCorrMap{3,jj}{8}(idxMouse,:),'Rows','pairwise','Type','Spearman')); 
            end
        else
            clear temp2 temp3
            for nn = 1:8
                tempA = zscore(PF.sessCorrMap{2,jj}{nn}(:,1:62),[],2);
                tempB = zscore(PF.sessCorrMap{2,jj}{nn}(:,63:95),[],2);
                temp2{nn} = [tempA tempB];
                
                tempA = zscore(PF.sessCorrMap{3,jj}{nn}(:,1:62),[],2);
                tempB = zscore(PF.sessCorrMap{3,jj}{nn}(:,63:95),[],2);
        ...
        for mm = 1:4%5
            ...
            for ss = 1:2
                ...
                %[PVstats{mm}.p(ss),~, PVstats{mm}.stats{ss}] = signrank(corrStats(:,ss));
            end           
            clear corrStats
        end
        ylim([-0.45 0.2])
        ...
        subplot(2,4,8) 
        stats.stem = groupStats(sessCorrmat,sessCorrManip,'inAxis',true,'color',[colMat;0.5 0.5 0.5]);
        title('Stem')
        [stats.stem.signrank.p,~,stats.stem.signrank.stats] = signrank(sessCorrmat(sessCorrManip==5),sessCorrmat(sessCorrManip==6));
        
        stats.PVcorr = PVstats;
        if savePlot
            if ~useZScore && ~downsample
                saveas(figure(jj),strcat(parentDir,'Compiled\Place Fields\PlaceField_',tag,'resorted.png'));
                saveas(figure(jj),strcat(parentDir,'Compiled\Place Fields\PlaceField_',tag,'resorted.eps'),'epsc');
                saveas(figure(jj),strcat(parentDir,'Compiled\Place Fields\PlaceField_',tag,'resorted.fig'));
                save(strcat(parentDir,'Compiled\Place Fields\PlaceFieldSessCorr',tag,'resorted.mat'),'stats');
            elseif downsample
