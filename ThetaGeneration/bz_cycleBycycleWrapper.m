...
function lfpCycle = bz_cycleBycycleWrapper(lfp, varargin)
...
%
% USAGE
%   lfpCycle = bz_cycleBycycle (lfp, <options>)
%
%
% INPUT
...
%% Conpute event-triggered LFP

events = events((events + twin(2) <= size(data,1)) & (events - twin(1) > 0));
lfp_event = nan(twin(1)+twin(2)+1,length(channels),length(events));

for e = 1:length(events)
    lfp_event(:,:,e) = data(events(e)-twin(1):events(e)+twin(2),channels);
end
