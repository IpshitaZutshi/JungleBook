...
function [ csd, lfpAvg ] = bz_eventThetaCSD (lfp, events, varargin)
...
% Now, only select peaks after 200ms & <5 s of the light on, and 200 ms
% before and <5 s before light on
idx_pre = [];
idx_post = [];
for ii = 1:length(peakIdx)
    for jj = 1:length(events)
        if peakIdx(ii) > (events(jj)-(5*samplingRate)) && peakIdx(ii) < (events(jj)-(0.2*samplingRate))
            idx_pre = [idx_pre; peakIdx(ii)];
        elseif peakIdx(ii) < (events(jj)+(5*samplingRate)) && peakIdx(ii) > (events(jj)+(0.2*samplingRate))
            idx_post = [idx_post; peakIdx(ii)];
        end
    end
end
...
lfp_pre = nan(twin(1)+twin(2)+1,length(channels),length(idx_pre));
% 
for e = 1:length(idx_pre)
    lfp_pre(:,:,e) = data(idx_pre(e)-twin(1):idx_pre(e)+twin(2),channels);
end
 
lfp_avg_pre = nanmean(lfp_pre,3)*-1;
num_pre = length(idx_pre);

lfp_post = nan(twin(1)+twin(2)+1,length(channels),length(idx_post));
% 
for e = 1:length(idx_post)
    lfp_post(:,:,e) = data(idx_post(e)-twin(1):idx_post(e)+twin(2),channels);
end
 
lfp_avg_post = nanmean(lfp_post,3)*-1;
num_post = length(idx_post);

%% Conpute CSD

% detrend
if doDetrend
   lfp_avg_pre = detrend(lfp_avg_pre')';
   lfp_avg_post = detrend(lfp_avg_post')';
end
    
% temporal smoothing
if temp_sm > 0
   for ch = 1:size(lfp_avg_pre,2) 
       lfp_avg_pre(:,ch) = smooth(lfp_avg_pre(:,ch),temp_sm,'sgolay');
       lfp_avg_post(:,ch) = smooth(lfp_avg_post(:,ch),temp_sm,'sgolay');
   end
end

% spatial smoothing
if spat_sm > 0
   for t = 1:size(lfp_avg_pre,1) 
       lfp_avg_pre(t,:) = smooth(lfp_avg_pre(t,:),spat_sm,'lowess');
       lfp_avg_post(t,:) = smooth(lfp_avg_post(t,:),spat_sm,'lowess');
   end
end

% calculate CSD 
CSD_pre = diff(lfp_avg_pre,2,2);
CSD_post = diff(lfp_avg_post,2,2);

% generate output structure
csd.data_pre = CSD_pre;
csd.data_post = CSD_post;
...
csd.num = [num_pre num_post];

lfpAvg.data_pre = lfp_avg_pre;
lfpAvg.data_post = lfp_avg_post;
