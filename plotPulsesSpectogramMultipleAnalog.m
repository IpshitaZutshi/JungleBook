function plotPulsesSpectogramMultipleAnalog(varargin)
...
[pulses] = bz_getAnalogPulsesSine('analogCh',64);
...
for i = 1:size(pulses.intsPeriodsSess,2)
    ...
    for ll = 1:length(startTS)
        ...
        plot([startTS(ll) startTS(ll)], [0 amp(1)],'b')
        plot([endTS(ll) endTS(ll)], [0 amp(1)],'k')
    end
