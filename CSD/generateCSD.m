...
%Generate matrix for CSD calculation
D = generateFmatrix(NumChannels);
...
imagesc(CSD');
ylim([2.5 NumChannels-1.5])
hold on
for i = 3:(NumChannels-2)
    plot((-(avgLFPsig(:,i)/(2*max(avgLFPsig(:))))+i),'k')
end
if stimSess == 1
    subplot(2,1,2)
...
function D = generateFmatrix(NumChannels)
