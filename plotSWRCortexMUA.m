...
if cond == 0
    ...
    ctxCh = {{19 54},{19 54},{19 54},{27 49},{8 56},...
        {0 54},{31 57},{27 40},...%{30 49},{5 53},
        {4 55},{19 58},{4 37 104},...{4 37 104},{0 50 99},
        {40},{63 104},{48 99}};
    analogCh = 2;
elseif cond == 1
...
if exist(strcat('Z:\Homes\zutshi01\Recordings\CA1_silencing\Compiled\Ripples\DownState\ripUnitData',num2str(cond),'.mat'),'file') && ~force
    load(strcat('Z:\Homes\zutshi01\Recordings\CA1_silencing\Compiled\Ripples\DownState\ripUnitData',num2str(cond),'.mat'))
else
    for ii = analogCh
    ...
    for i=1:size(pathToSessionsAll,2)
        ...
        for aa = analogCh
            ...
            keepIdx(:,1) = InIntervals(ripTS,events-5);    
            keepIdx(:,2) = InIntervals(ripTS ,events);           
            keepIdx(:,3) = InIntervals(ripTS ,events+5);       
            keepIdx = logical(keepIdx);

            if sum(keepIdx(:,1))>2 && sum(keepIdx(:,2))>2 && sum(keepIdx(:,3))>2
                [stccg, t] = CCG({MUAts ripTS(keepIdx(:,1))},[],'binSize',0.01,'duration',1,'norm','rate');            
            ...
            if sum(keepIdx(:,1))>2 && sum(keepIdx(:,2))>2 && sum(keepIdx(:,3))>2
                [stccg, t] = CCG({MUAts ripTS(keepIdx(:,2))},[],'binSize',0.01,'duration',1,'norm','rate');            
            ...
            if sum(keepIdx(:,1))>2 && sum(keepIdx(:,2))>2 && sum(keepIdx(:,3))>2
                [stccg, t] = CCG({MUAts ripTS(keepIdx(:,3))},[],'binSize',0.01,'duration',1,'norm','rate');            
    ...
    if saveMat
        save(['Z:\Homes\zutshi01\Recordings\CA1_silencing\Compiled\Ripples\DownState\ripUnitData',num2str(cond),'.mat'], ...
...
%     stdpsth = nanstd(zscore(distRipStim{ii},[],2),1)./sqrt(size(distRipStim{ii},1));   
    ...
    idxTS = t<0.03 & t>-0.09;
    temp = distRipPre{ii};%zscore(distRipPre{ii},[],2);
    data{1} = nanmean(temp(:,idxTS),2);
    temp = distRipStim{ii};%zscore(distRipStim{ii},[],2);
    data{2} = nanmean(temp(:,idxTS),2);
    temp = distRipPost{ii};%zscore(distRipPost{ii},[],2);
    data{3} = nanmean(temp(:,idxTS),2);
    stats{ii} = groupStats(data,[],'repeatedMeasures',true,'plotType','boxLinesSEM','inAxis',true);
    
end

saveas(gcf,strcat(expPath,'Compiled\Ripples\DownState\RippleSpiking_',num2str(cond),'.png'));
saveas(gcf,strcat(expPath,'Compiled\Ripples\DownState\RippleSpiking_',num2str(cond),'.eps'),'epsc');
saveas(gcf,strcat(expPath,'Compiled\Ripples\DownState\RippleSpiking_',num2str(cond),'.fig'));
save(strcat(expPath,'Compiled\Ripples\DownState\RippleSpiking_',num2str(cond),'.mat'),'stats');  
