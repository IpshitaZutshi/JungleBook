...
len = 406679; % 13116160 for PFC, 12836608 for hippo
% t_end = 437.205333333; % prefrontal
%t_end = 427.886933; % hippocampus
% time = 0:1/Fs:t_end;
% shortened_time = time(:,1:len);
% time = downsample(shortened_time,Fs/1250);

F_green=v(1,:);
F_lp = filtfilt(b,a,F_green);
shortened_green = F_lp(:,1:len);
F_lp = downsample(shortened_green,Fs/1250);

F_red=v(2,:);
F_red_lp = filtfilt(b,a,F_red);
shortened_red = F_red_lp(:,1:len);
F_red_lp = downsample(shortened_red,Fs/1250);


%F_noise=v(3,:);
%F_noise_lp = filtfilt(b,a,F_noise);
%F_noise_lp = downsample(F_noise_lp,Fs/1250);

%{
hold on
plot(time,F_lp);
plot(time,F_red_lp);
%plot(time,F_noise_lp);
hold off
%}
...
% tr = 1:1:length(F_red_lp);
% br = polyfit(1:1:length(F_red_lp),F_red_lp,5);
% ar = polyval(br,tr);
% dfByF_red = (F_red_lp-ar)./ar;
F_red_lp = detrend(F_red_lp);
