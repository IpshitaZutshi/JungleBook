...
if doPlot
    ...
    elseif strcmpi(plotType,'medianBall')
        ...
        for ii = 1:length(pos)
            plot([pos(ii) pos(ii)], abs([stats.descriptive.q25(ii) stats.descriptive.q75(ii)]),'color',color(1,:));
        end
    ...
    elseif strcmpi(plotType,'fillStd') || strcmpi(plotType,'fillSEM')
        ...
        if plotData
            for ii = 1:length(ind) 
                ...
                'MarkerSize',3,'MarkerEdgeColor','none','MarkerFaceColor',dataColor);
            end
        end
        xlim([.5 max(pos)+.5]);
        set(gca,'xtick',[]);
