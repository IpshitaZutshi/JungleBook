function SessBehaviorCoherenceRad(varargin)
...
if exist('Summ\cohData.mat','file') && ~force 
    disp('Theta rad coherence already computed! Loading file.');
    load('Summ\cohData.mat');
else
    for rr = 1:3
        for cc = 1:2
            for zz = 1:6
    ...
    for ii = 1:size(allSess,1)
        ...
        if sum(continueAnalysis)>0
            ...
            coherenceAll = bz_lfpCoherence(radCh,lfp,'forceDetect',true,'showfig',false,'saveMat',true);         
            ...
            for jj = 1:length(efields)
                if continueAnalysis(jj) ==1
                    ...
                    if makePlots   
                        ...
                        saveas(figure(1),[efields{jj} '\Analysis\RadCoherence.png'],'png');
                        saveas(figure(1),[efields{jj} '\Analysis\RadCoherence.fig'],'fig');
                        saveas(figure(1),[efields{jj} '\Analysis\RadCoherence.eps'],'epsc');

                        saveas(figure(2),[efields{jj} '\Analysis\RadCohPhase.png'],'png');
                        saveas(figure(2),[efields{jj} '\Analysis\RadCohPhase.fig'],'fig');
                        saveas(figure(2),[efields{jj} '\Analysis\RadCohPhase.eps'],'epsc');             
                    end
                    ...
                    if saveSessMat
                        ...
                        save([efields{jj} '\Analysis\Coherence.mat'], 'coherence');
                    end
                    close all
    ...
    if saveMat 
        save([expPath '\Summ\' 'cohData.mat'], 'csdcohData');
    end
end
end
