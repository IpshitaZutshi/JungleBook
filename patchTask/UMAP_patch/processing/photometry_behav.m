...
for kk = 1:length(fieldnames(photometry_data))
    ...
    for k = 1:length(low_patch)
        if behavTrials.patch_trials(low_patch(k, 2)) >= 30
            ...
            if start_idx >= 1 && end_idx <= length(photom_var.timestamps)
                ...
                % using -4s to -1.6s before ripple as baseline
                % period
                baseline_period = zscore_matrix_low(k, 130:440); % Define baseline window before ripple event
                baseline = baseline_period;
    ...
    for k = 1:length(sswitch)
        ...
        if start_idx >= 1 && end_idx <= length(photom_var.timestamps)
            ...
            % using -4s to -1.6s before ripple as baseline
            % period
            baseline_period = zscore_matrix_switch(k, 130:440); % Define baseline window before ripple event
            baseline = baseline_period;
    ...
    for j = 1:length(unexpected_reward)
        ...
        if start_idx >= 1 && end_idx <= length(photom_var.timestamps)
            ...
            % using -3.1s to -1s before ripple as baseline
            % period
            baseline_period = zscore_matrix_unexpected(j, 1:270); % Define baseline window before ripple event
            baseline = baseline_period;
            % Calculate baseline mean and std
            baseline_mean = mean(baseline);
            baseline_std = std(baseline);
    ...
    for j = 1:length(unexpected_non)
        ...
        if start_idx >= 1 && end_idx <= length(photom_var.timestamps)
            ...
            % using -3.1s to -1s before ripple as baseline
            % period
            baseline_period = zscore_matrix_unexpected_non(j, 1:270); % Define baseline window before ripple event
            baseline = baseline_period;
            % Calculate baseline mean and std
            baseline_mean = mean(baseline);
            baseline_std = std(baseline);
