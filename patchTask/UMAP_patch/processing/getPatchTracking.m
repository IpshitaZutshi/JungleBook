         ...
         if ~isempty(dir([basepath filesep MergePoints.foldernames{ii} filesep 'top*']))   
            ...
            for i = 1:length(photometry_file)
                ...
                if contains(lower(fname), 'hpc')
                    photomBehav_hpc{count} = getSyncPhotometry_IZ(data.photometryData);
                    photometry_exists(1) = 1;
                elseif contains(lower(fname), 'striatum')
                    photomBehav_striatum{count} = getSyncPhotometry_IZ(data.photometryData);
...
if exist([basepath filesep strcat(sessionInfo.session.name,'.MergePoints.events.mat')],'file')
    ...
    for ii = 1:length(trackFolder)
        if strcmpi(MergePoints.foldernames{trackFolder(ii)},behavTracking{ii}.folder)
            ...
            if photometry_exists(1) == 1
                sumTsPhot = photomBehav_hpc{ii}.timestamps + MergePoints.timestamps(trackFolder(ii),1); %%%
                tsPhot_hpc = [tsPhot_hpc; sumTsPhot]; %%%
            end
            
            if photometry_exists(2) == 1
                sumTsPhot = photomBehav_striatum{ii}.timestamps + MergePoints.timestamps(trackFolder(ii),1); %%%
                tsPhot_striatum = [tsPhot_striatum; sumTsPhot]; %%%
            end
...
if photometry_exists(1) == 1
    photometry_hpc = photomBehav_hpc{1};
    photometry_hpc.timestamps = tsPhot_hpc;
end

if photometry_exists(2) == 1
    photometry_striatum = photomBehav_striatum{1};
    photometry_striatum.timestamps = tsPhot_striatum;
end
...
if saveMat
    ...
    if photometry_exists(1) == 1
        save([basepath filesep sessionInfo.FileName '.PhotometryBehavHPC.mat'],'photometry_hpc');
    end
    if photometry_exists(2) == 1
        save([basepath filesep sessionInfo.FileName '.PhotometryBehavStriatum.mat'],'photometry_striatum');
    end
end
