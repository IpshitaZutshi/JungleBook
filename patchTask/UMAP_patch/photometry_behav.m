...
window = 0.9; % window of time around ripple to average
samples = window*sampling_rate;

% need to find end timestamp of first sleep session. will write code for
% this, but for right now can just use MergePoints

sleep_start = 12416; % start of sleep session
sleep_end = 19840; % time stamp of end of sleep session 
...
xlabel('time (s)');
ylabel('avg z-score');
title('Average Z-score Around Ripples (Post-task Sleep)');
grid on;
hold off
