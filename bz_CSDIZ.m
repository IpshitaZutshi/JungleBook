...
function [ csd ] = bz_CSDIZ (lfp, varargin)
...
if exist(strcat(lfp.Filename(1:(end-4)),'.csd.mat'),'file') && ~force 
    disp('CSD already computed! Loading file.');
    load(strcat(lfp.Filename(1:(end-4)),'.csd.mat'));
    return
end
...
lfp_frag = double(data(win(1):win(2),1:length(channels)))*-1;
