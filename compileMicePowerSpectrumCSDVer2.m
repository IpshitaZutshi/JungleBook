...
function compileMicePowerSpectrumCSDVer2(varargin)
...
if strcmp(tag,'CA1') == 1
    mice = {'IZ18\Final','IZ20\Final','IZ31\Final','IZ21\Final'};%
...
chName = {'pyramidal','slm'};%,'radiatum','ml'};
    ...
    for ch = 1:length(chName)
        for ii = 1:3
            for jj = 1%:2
                ...
                mouseID = [];
                for kk = 1:6  
                    if strcmp(mice{m},'IZ31\Final')==1
                        ppLength = 2;
                    else
                        ppLength = size(PSS.(chName{ch}).specGram{ii,jj}{kk},3);
                    end
                    for pp = 2:ppLength % First array is nans                        
                        spec = (10.^PSS.(chName{ch}).specGram{ii,jj}{kk}(:,:,pp));
                        specGram.(chName{ch}){ii,jj}{kk} = [specGram.(chName{ch}){ii,jj}{kk};nanmedian(spec,1)];
                        freqIdx = freqs>6 & freqs<12;
                        thetaPwr(pp-1,kk) = nanmedian(nanmedian(spec(:,freqIdx),1));
                        freqIdx = freqs>25 & freqs<90;
                        gammaPwr(pp-1,kk) = nanmedian(nanmedian(spec(:,freqIdx),1));
                        mouseID(pp-1,1) = m;
                    end  
                end
        ...
        for ii = 1:3
            for jj = 1:2
                for m = unique(mouse.(chName{ch}){ii,jj})'
                    ...
                    for zz = 1:6                    
                        meanspec = nanmedian(specGram.(chName{ch}){ii,jj}{zz}(mID,:),1);
                        specGramComb.(chName{ch}){ii,jj}{zz} = [specGramComb.(chName{ch}){ii,jj}{zz};meanspec];    
                    end
                    thetapowerComb.(chName{ch}){ii,jj} = [thetapowerComb.(chName{ch}){ii,jj};nanmedian(thetapower.(chName{ch}){ii,jj}(mID,:),1)];
                    gammapowerComb.(chName{ch}){ii,jj} = [gammapowerComb.(chName{ch}){ii,jj};nanmedian(gammapower.(chName{ch}){ii,jj}(mID,:),1)];
                end
            end
        end
    end
        ...
        for jj = 1%:length(target)
            for kk = [1 2]   
                if combineSessions
                    ...
                    thetamEC = thetapower.(chName{ch}){2,jj}(:,kk+3)-thetabase;
                    thetaCA3 = thetapower.(chName{ch}){3,jj}(:,kk)-thetabase;
                    thetaBoth = thetapower.(chName{ch}){3,jj}(:,kk+3)-thetabase;
                    
                    gammabase = gammapower.(chName{ch}){2,jj}(:,kk);
                    gammamEC = gammapower.(chName{ch}){2,jj}(:,kk+3)-gammabase;
                    gammaCA3 = gammapower.(chName{ch}){3,jj}(:,kk)-gammabase;
                    gammaBoth = gammapower.(chName{ch}){3,jj}(:,kk+3)-gammabase;                             
                end
...
%                     'inAxis',true,'repeatedMeasures',true,'labelSummary',false,'Color',colMat);
                stats{kk}.(chName{ch}).theta = groupStats([thetaCA3;thetamEC;thetaBoth],[ones(length(thetaCA3),1)*2;ones(length(thetamEC),1)*3;ones(length(thetaBoth),1)*4],...
                    'inAxis',true,'repeatedMeasures',true,'labelSummary',false,'Color',colMat);
                [stats{kk}.(chName{ch}).theta.signrank.p(1),~,stats{kk}.(chName{ch}).theta.signrank.stats{1}] = signrank(thetaCA3);    
                [stats{kk}.(chName{ch}).theta.signrank.p(2),~,stats{kk}.(chName{ch}).theta.signrank.stats{2}] = signrank(thetamEC);   
                [stats{kk}.(chName{ch}).theta.signrank.p(3),~,stats{kk}.(chName{ch}).theta.signrank.stats{3}] = signrank(thetaBoth);                   
                title('theta power')
...
%                     'inAxis',true,'repeatedMeasures',true,'labelSummary',false,'Color',colMat);
                stats{kk}.(chName{ch}).gamma = groupStats([gammaCA3;gammamEC;gammaBoth],[ones(length(gammaCA3),1)*2;ones(length(gammamEC),1)*3;ones(length(gammaBoth),1)*4],...
                    'inAxis',true,'repeatedMeasures',true,'labelSummary',false,'Color',colMat);
                [stats{kk}.(chName{ch}).gamma.signrank.p(1),~,stats{kk}.(chName{ch}).gamma.signrank.stats{1}] = signrank(gammaCA3);    
                [stats{kk}.(chName{ch}).gamma.signrank.p(2),~,stats{kk}.(chName{ch}).gamma.signrank.stats{2}] = signrank(gammamEC);   
                [stats{kk}.(chName{ch}).gamma.signrank.p(3),~,stats{kk}.(chName{ch}).gamma.signrank.stats{3}] = signrank(gammaBoth);   
                title('gamma power')
            end
        end
    end
            ...
            for ii = 1:length(reg)
                for jj = 1%:length(target)
                    if combineSessions
                        ...
                        thetastim{ii} = thetapower.(chName{ch}){ii,jj}(:,kk+3)-thetapower.(chName{ch}){ii,jj}(:,kk);
                        %gammabase{kk} = [gammabase{kk};gammapower.(chName{ch}){ii,jj}(:,kk)];
                        gammastim{ii} = gammapower.(chName{ch}){ii,jj}(:,kk+3)-gammapower.(chName{ch}){ii,jj}(:,kk);                               
                    end
...
%             'inAxis',true,'labelSummary',false,'Color',colMat);
            stats{kk}.(chName{ch}).theta = groupStats([thetastim{1};thetastim{2};...
            thetastim{3}],[ones(length(thetastim{1}),1);ones(length(thetastim{2}),1)*2;ones(length(thetastim{3}),1)*3],...   
            'inAxis',true,'labelSummary',false,'Color',colMat);  
            for ll = 1:3
                [stats{kk}.(chName{ch}).theta.signrank.p(ll),~,stats{kk}.(chName{ch}).theta.signrank.stats{ll}] = signrank(thetastim{ll});
            end
            title('theta power')
...
%             'inAxis',true,'labelSummary',false,'Color',colMat);
            stats{kk}.(chName{ch}).gamma = groupStats([gammastim{1};gammastim{2};...
            gammastim{3}],[ones(length(gammastim{1}),1);ones(length(gammastim{2}),1)*2;ones(length(gammastim{3}),1)*3],... 
            'inAxis',true,'labelSummary',false,'Color',colMat);        
            for ll = 1:3
                [stats{kk}.(chName{ch}).gamma.signrank.p(ll),~,stats{kk}.(chName{ch}).gamma.signrank.stats{ll}] = signrank(gammastim{ll});
            end        
            title('gamma power')
