function [f_peak, power_ratio,f, S] = IntrinsicFrequency(self, cel, print_to_screen, speed_thresh, varargin)
...
for epoch_ind = 1%:size(orig_epoch, 1)
    ...
    %self.epoch = orig_epoch(epoch_ind,:); Original
    self.epoch = orig_epoch;
    
    max_lag_t = .6; 

    t_bin = .005; % 2 ms bin width   
    
    [cor, lag] = self.AutoCorr(cel, 'speed_thresh', speed_thresh, 'theta_skip', 0, 't_bin', t_bin, 'supress_plot', 1, 'average_epochs', 1);
    ...
    [a_peak,ind] = max(stmp(ftmp > 7 & ftmp < 25)); % intrinsic frequency

    ftmp = ftmp(ftmp > 6 & ftmp < 25);
...
%keyboard
    if print_to_screen
        ...
        line(f(f>=0 & f<=25), 10^9*S(f>=0 & f<=25), 'Color', 'k', 'LineWidth', 2), hold on;
