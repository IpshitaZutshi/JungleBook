function compileMicePhaseLockingVer2
...
%             title('Side arm')
        ...
        saveas(figure(1),strcat(parentDir,'\Compiled\Phase Locking\',tag,'\PhaseLockingStats',taglfp,'.png'),'png');
        saveas(figure(1),strcat(parentDir,'\Compiled\Phase Locking\',tag,'\PhaseLockingStats',taglfp,'.fig'),'fig');
        saveas(figure(1),strcat(parentDir,'\Compiled\Phase Locking\',tag,'\PhaseLockingStats',taglfp,'.eps'),'epsc');
        save(strcat(parentDir,'\Compiled\Phase Locking\',tag,'\PhaseLockingStats',taglfp,'.mat'),'stats');
    else
        for ii = 1:length(reg)
            for jj = 1%:length(target)
                % Extract cell id and region info
