function manifoldPlot_error(varargin)
...
for tt = 1:length(TRIAL_TYPE)
    if TRIAL_TYPE(tt)<6 && exist('correct_ds')
        if exist('probe_ds')
            plot_ind =  [plot_ind,find(lick_loc_ds==TRIAL_TYPE(tt) & correct_ds==0 & probe_ds==probe)];
        else
            plot_ind =  [plot_ind,find(lick_loc_ds==TRIAL_TYPE(tt) & correct_ds==0)]; 
        end
    else
