...
for tt = 1:length(trials) % Make separate videos for each trial
    ...
    subplot(numrows,numcol,[25 26 31 32]) % Position UMAP - static
    set(gca,'visible','off')

    subplot(numrows,numcol,[27 28 33 34]) % Goal UMAP - static
    set(gca,'visible','off')
    
    subplot(numrows,numcol,[29 30 35 36]) % Single trial UMAP
    set(gca,'visible','off')
    ...
    for ii = startFrame:endFrame
        ...
        h5 = scatter(tracking.position.x(ii),post_pos(posDec),60,'m','filled');
        ...
        %% Plot  manifold
        if ii == startFrame
            plotAvgManifold(umap_name,behav_file,numrows,numcol,[25 26 31 32],A,E,fig,1)
            plotAvgManifold(umap_name,behav_file,numrows,numcol,[27 28 33 34],A,E,fig,2)
        end
            
        videoManifold(umap_name,behav_file,numrows,numcol,[29 30 35 36],A,E,fig,[tracking.timestamps(startFrame) tracking.timestamps(ii)])                
        ...
        if sum(ismember(spikeData.posIdx{cell1}, ii))>0
            ...
            hold on
            scatter(tracking.timestamps(ii),tracking.position.y(ii),30,'r',"filled")
        end
        if sum(ismember(spikeData.posIdx{cell2}, ii))>0
            ax1 = subplot(numrows,numcol,43:48);
            hold on
            scatter(tracking.timestamps(ii),tracking.position.y(ii),30,'yellow',"filled")
        end
...
for tt = 1:length(TRIAL_TYPE)
    plot_ind =  [plot_ind,find(lick_loc_ds==TRIAL_TYPE(tt) & correct_ds==1 & probe_ds==0 & speed_all'>2)];   
end
...
function videoManifold(umap_name,behav_file,numrows,numcol,plotloc,A,E,figHandle,win)
...
scatter3(Umap_results(plot_ind,1),Umap_results(plot_ind,2),Umap_results(plot_ind,3),5,[0.85 0.85 0.85],'filled','MarkerFaceAlpha',0.05);
