function FigS9_ReturnRunPlaceFields
...
for ii = 1:3            
    ...
    [~,idxInCorr] = max(selectedMapInCorr,[],2);   
    ...
    if ii == 1
        subplot(numrows,numcol,numcol*3+1);  %% Distribution of place cells
        histogram(idxLin, 1:1:50,'DisplayStyle','stairs','LineWidth',1.5,'Normalization','probability')    
        ylim([0 0.1])
    elseif ii == 2
        subplot(numrows,numcol,numcol*3+2);  %% Distribution of place cells
        histogram(idxSpace, 1:1:50,'DisplayStyle','stairs','LineWidth',1.5,'Normalization','probability') 
        ylim([0 0.1])
        subplot(numrows,numcol,numcol*3+3);  %% Distribution of place cells
        histogram(idxInCorr, 1:1:50,'DisplayStyle','stairs','LineWidth',1.5,'Normalization','probability') 
        ylim([0 0.1])
    else
        subplot(numrows,numcol,numcol*3+4);  %% Distribution of place cells
        histogram(idxLinEnd, 1:1:50,'DisplayStyle','stairs','LineWidth',1.5,'Normalization','probability')   
        ylim([0 0.1])
    end
end   
...
saveas(gcf,strcat(expPath,'SupFigure9A_returnRun.png'));
saveas(gcf,strcat(expPath,'SupFigure9A_returnRun.eps'),'epsc');
saveas(gcf,strcat(expPath,'SupFigure9A_returnRun.fig'));
