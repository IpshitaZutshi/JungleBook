...
for ii = 1:numel(trialNum)     
    ...
    for k = 1:(numel(troughIdx)-2)
        ...
        lookPhase = mod(lfp_phase(i1:i2), 2*pi);
...
% --- Simple smoothed phase × lookahead plot (2 cycles) ---

% Duplicate phase to show 2 theta cycles
base_phase = [base_phase_all; base_phase_all + 2*pi];
base_dlt   = [base_dlt_all;   base_dlt_all];

stim_phase = [stim_phase_all; stim_phase_all + 2*pi];
stim_dlt   = [stim_dlt_all;   stim_dlt_all];

% Binning
phaseEdges = linspace(0, 4*pi, 80);
dltEdges   = linspace(-25, 25, 80);

% 2D histograms (occupancy)
Hb = histcounts2(base_dlt, base_phase, dltEdges, phaseEdges);
Hs = histcounts2(stim_dlt, stim_phase, dltEdges, phaseEdges);

% Normalize
Hb = Hb / max(sum(Hb(:)),1);
Hs = Hs / max(sum(Hs(:)),1);

% Smooth (simple Gaussian)
Hb = smooth2D(Hb, 1.2);
Hs = smooth2D(Hs, 1.2);

% Axes
phaseC = (phaseEdges(1:end-1)+phaseEdges(2:end))/2;
dltC   = (dltEdges(1:end-1)+dltEdges(2:end))/2;

% Plot
figure;
tiledlayout(1,2,'TileSpacing','compact','Padding','compact')

cl = [min([Hb(:);Hs(:)]), max([Hb(:);Hs(:)])];
levels = linspace(cl(1),cl(2),20);

nexttile
contourf(phaseC, dltC, Hb, levels, 'LineColor','none');
set(gca,'YDir','normal')
xticks([0 2*pi 4*pi]); xticklabels({'0','2\pi','4\pi'})
xlabel('Theta phase'); ylabel('Decoded - true')
title('Baseline'); colormap(jet); caxis(cl); colorbar

nexttile
contourf(phaseC, dltC, Hs, levels, 'LineColor','none');
set(gca,'YDir','normal')
xticks([0 2*pi 4*pi]); xticklabels({'0','2\pi','4\pi'})
xlabel('Theta phase'); ylabel('Decoded - true')
title('Stim'); colormap(jet); caxis(cl); colorbar
