...
Fig4Stats.lickType = groupStats([{avgRate(:,1)},{avgRate(:,2)},{avgRate(:,3)},{avgRate(:,4)},{avgRate(:,5)}],[],'Color',col,'inAxis',true,'labelSummary',false);
...
Fig4Stats.correctIncorrect = groupStats([{avgRate(:,1)},{avgRate(:,2)}],[],'doPlot',false);
...
for tt = 1:length(lickT)
    ...
    for ss = 1:max(Summary.sessID)
        ...
        numProp(ss,tt) = sum(avgRate>0.5)./length(avgRate);
    end   
end
...
subplot(numrows,numcol,plotloc)
if ~usePGAM
    Fig4Stats.portFract = groupStats([{numProp(:,1)},{numProp(:,2)},{numProp(:,3)},{numProp(:,4)},{numProp(:,5)},{numProp(:,6)}],[],'Color',col,'inAxis',true,'labelSummary',false);
else
    %Fig4Stats.portFract = groupStats([{numProp(:,1)},{numProp(:,2)},{numProp(:,3)},{numProp(:,4)},{numProp(:,5)},{numProp(:,6)},{numProp(:,7)}],[],'Color',col,'inAxis',true,'labelSummary',false);
    Fig4Stats.portFract = groupStats([{numProp(:,1)},{numProp(:,2)},{numProp(:,3)}],[],'Color',col,'inAxis',true,'labelSummary',false);
end
...
if ~usePGAM
    ...
    save(strcat(expPath,'\Compiled\Figures_Sep23\Figure5_lickCells.mat'),'Fig4Stats'); 
else
    saveas(gcf,strcat(expPath,'\Compiled\Figures_Sep23\Figure5_lickCellsPGAM.png'));
    saveas(gcf,strcat(expPath,'\Compiled\Figures_Sep23\Figure5_lickCellsPGAM.eps'),'epsc');
    saveas(gcf,strcat(expPath,'\Compiled\Figures_Sep23\Figure5_lickCellsPGAM.fig'));
    save(strcat(expPath,'\Compiled\Figures_Sep23\Figure5_lickCellsPGAM.mat'),'Fig4Stats'); 
end
