...
function compileMicePhasePrecessionVer2(varargin)
...
elseif strcmp(tag,'mEC') == 1
    mice = {'IZ12\Final','IZ13\Final','IZ15\Final','IZ17\Final','IZ18\Final','IZ20\Final'...
        ...
        'IZ30\Final','IZ31\Final','IZ32\Saline','IZ33\Saline'};  % To add: IZ34
    reg = {'CA1','mEC','Both'};
elseif strcmp(tag,'CA3') == 1
    mice = {'IZ34\Final','IZ32\Final','IZ28\Final','IZ27\Final','IZ33\Final'};%,,'IZ29\Final'
    reg = {'CA3','mEC','Both'};
...
if strcmp(tag,'CA1') == 1 || strcmp(tag,'mECBilateral') == 1 || strcmp(tag,'mEC') == 1
    ...
    for jj = 1%:length(target)
        ...
        for kk = 1:3       
           kkIdx = 2*(kk-1)+1;
           idxtoKeepAll = field_combined(:,kkIdx:kkIdx+1) >70 & field_combined(:,kkIdx:kkIdx+1) < 95;
           idxtoKeepSig = field_combined(:,kkIdx:kkIdx+1) >70 & field_combined(:,kkIdx:kkIdx+1) < 95 & p_combined(:,kkIdx:kkIdx+1) <0.05;
...
elseif strcmp(tag,'CA3') == 1 || strcmp(tag,'CA3Saline') == 1
    ...
    for jj = 1%:length(target)
       ...
       idxtoKeepAll = field_combined(:,:) >63 & field_combined(:,:) < 100;% & abs(slope_combined(:,:))<=1.5;
       idxtoKeepSig = field_combined(:,:) >63 & field_combined(:,:) <100 & p_combined(:,:) <0.05;% & abs(slope_combined(:,:))<=1.5;
       ...
       for mm = unique(miceBase)'
           ...
           avgSlopeBase(mm,1) = nanmean(slopesBase(idxBase));
           idxCA3 = miceCA3 == mm;
           avgSlopeCA3(mm,1) = nanmean(slopesCA3(idxCA3));                          
           idxStim = miceBoth == mm;
           avgSlopeBoth(mm,1) = nanmean(slopesBoth(idxStim));           
       end
