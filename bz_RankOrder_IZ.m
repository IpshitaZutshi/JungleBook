...
addParameter(p,'numRep',100,@isnumeric);
...
templateExt = p.Results.templateExt;
...
rankUnits = nan*ones(size(spkEventTimes.UnitAbs,2),size(spkEventTimes.UnitEventRel,2));
...
pvalEvents = zeros(nCond,size(corrEvents,2));
parfor event = 1:size(corrEvents,2)
