function compileMiceAssembliesVer2(varargin)
...
%                    end
                   ...
                   if ~isempty(currManip)
                       ...
                       elseif tag == 3
                           assemblyrat = currManip(:,1);abs(baseManip(:,2)-baseManip(:,1))./abs(baseManip(:,2)+baseManip(:,1));
                   ...
                   if ii ==1
                       currManip = Assemblies{jj}(:,1);%Assemblies{jj}(:,[5 6]);
                       assemblyrat = currManip;%abs(currManip(:,2)-currManip(:,1))./abs(currManip(:,2)+currManip(:,1));
                       compiledAssemblies{currIdx} = [compiledAssemblies{currIdx};assemblyrat];                            
                   else
                       currManip = Assemblies{jj}(:,ii);%Assemblies{jj}(:,[1 ii]);
                       if ~isempty(currManip)                   
                           assemblyrat = currManip;%abs(currManip(:,2)-currManip(:,1))./abs(currManip(:,2)+currManip(:,1));
                           compiledAssemblies{currIdx} = [compiledAssemblies{currIdx};assemblyrat];       
                       end                
